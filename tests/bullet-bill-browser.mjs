...
try {
  ...
  assert.equal(afterBomb.enemies.find(enemy => enemy.type === "bullet-bill").move.duration, 0.28125, "first actual missile step uses the 20% reduced speed");
  ...
  motionSamples.forEach(({ gx, gy, move, straightSteps }) => {
    assert.equal(move.duration, 0.28125, "all actual movement uses constant reduced speed");
