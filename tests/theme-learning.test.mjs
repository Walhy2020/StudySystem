...
test("主题页十二个卡片、九十一个互动目标、歌词音标、阶段隔离和缓存引用齐全", async () => {
  ...
  assert.match(html, /theme-learning\.js\?v=2\.12/);
  ...
  assert.match(script, /theme-overview\.js\?v=1\.9/);
...
test("总复习是独立并列模块，主题页只记录学习进度", async () => {
  ...
  assert.match(reviewHtml, /review-learning\.js\?v=1\.10/);
  assert.match(reviewHtml, /id="wordLibraryCount">0\/219<\/b>/);
  assert.match(reviewScript, /theme-learning\.js\?v=2\.12/);
  assert.match(reviewScript, /theme-overview\.js\?v=1\.9/);
